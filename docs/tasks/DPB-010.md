...
- [x] `internal/venueselector` パッケージを新規作成する。
- [x] `VenueSelector` インターフェースと、ランダムに選定する `RandomVenueSelector` を実装する。
- [x] `config.VenueConfig` のスライスを受け取り、ランダムに1つの `config.VenueConfig` を返す `Select` メソッドを実装する。
- [x] ユニットテストを実装し、リストの中からランダムに1つが選ばれることを確認する。
...
- [x] `internal/venueselector` パッケージ
- [x] `RandomVenueSelector` の実装とユニットテスト
