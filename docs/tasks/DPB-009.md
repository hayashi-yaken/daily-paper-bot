...
- [ ] `internal/config` の `Config` 構造体を変更し、単一のVenue情報ではなく、Venue情報のスライス（配列）を保持できるようにする。
- [ ] 新しい環境変数 `OR_VENUES_JSON` を定義し、そのJSON文字列をパースして設定に読み込むロジックを実装する。
- [ ] これまでの `OR_VENUE`, `OR_YEAR`, `OR_VENUE_URL` は廃止する。
- [ ] `main.go` で設定を読み込んでいる部分のコンパイルエラーを一時的に修正する（後続のチケットで本格的に修正）。
...
- [ ] 複数学会を扱えるように更新された `internal/config` パッケージ
- [ ] ユニットテストの修正
