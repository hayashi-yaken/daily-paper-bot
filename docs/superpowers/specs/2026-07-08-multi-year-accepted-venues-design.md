...
なお、この窓方式は**厳密な一様サンプリングではない**。offset は一様に選ばれるため中央部の論文の選定確率は互いに等しいが、リストの先頭・末尾の約 19 件は窓に含まれる確率がやや低くなる(エッジバイアス)。数千件規模の採択論文に対して影響は軽微であり、無効データ耐性を優先した実用的な方式として採用する。
